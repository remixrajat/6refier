...
class MyTrackingCourse extends Component {
    ...
    render() {
        console.log("MyTrackingCourse::props", this.props)
        let content = []
        let test = []
        let documents = []
        let packageDef = []
        ...
        for (let i = 0; i < packageMappings.length; i++){
            if (packageMappings[i].is_assigned) {
                ...
                for (let i = 0; i < permissions.length; i++){
                    // if (permissions[i].user_profile.length > 0) {
...
var mapStateToProps =  (store, ownProps) => {
  ...
  export default connect(mapStateToProps)(MyTrackingCourse);